...
httpServer.onComplete{httpServerFuture.completer()}

var netServerFuture = Future.future()
netServer.onComplete{netServerFuture.completer()}

CompositeFuture.all(httpServerFuture, netServerFuture).onComplete{
  case Success(result) => {
    // All servers started
  }
...
----
import io.vertx.scala.core.CompositeFuture
CompositeFuture.join(future1, future2, future3).onComplete{
  case Success(result) => {
    // All succeeded
  }
  case Failure(throwable) => {
...
fs.onComplete{fut1.completer()}

fut1.compose((v: java.lang.Void) => {
  // When the file is created (fut1), execute this:
  var fut2 = Future.future()
  fs.onComplete{fut2.completer()}
  return fut2
}).compose((v: java.lang.Void) => {
  // When the file is written (fut2), execute this:
  fs.onComplete{startFuture.completer()}
}, startFuture)

...
vertx.executeBlocking((future: io.vertx.scala.core.Future<java.lang.Object>) => {
...
executor.executeBlocking((future: io.vertx.scala.core.Future<java.lang.Object>) => {
  // Call some blocking API that takes a significant amount of time to return
  var result = someAPI.blockingMethod("hello")
  future.complete(result)
}, .onComplete {
...
CompositeFuture.all(httpServerFuture, netServerFuture).setHandler(.onComplete {
    case Success(result) => {
      // All servers started
    }
...
----
import io.vertx.scala.core.CompositeFuture
CompositeFuture.any(future1, future2).setHandler(.onComplete {
    case Success(result) => {
...
----
import io.vertx.scala.core.CompositeFuture
CompositeFuture.join(future1, future2, future3).setHandler(.onComplete {
    case Success(result) => {
      // All succeeded
    }
    case Failure(throwable) => {
...
[source,scala]
----
vertx.deployVerticle("com.mycompany.MyOrderProcessorVerticle", .onComplete {
    case Success(result) => {
      println(s"Deployment id is: ${res.result()}")
    }
    case Failure(throwable) => {

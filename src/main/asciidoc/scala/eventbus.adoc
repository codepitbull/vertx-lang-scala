...
[source,scala]
----
consumer.completionHandler(.onComplete {
    case Success(result) => {
...
[source,scala]
----
consumer.unregister(.onComplete {
    case Success(result) => {
...
[source,scala]
----
eventBus.send("news.uk.sport", "Yay! Someone kicked a ball across a patch of grass", .onComplete {
    case Success(result) => {
      println(s"Received reply: ${ar.result().body()}")
    }

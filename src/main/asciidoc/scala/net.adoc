...
var server = vertx.createNetServer()
server.listen(1234, "localhost", .onComplete {
    case Success(result) => {
...
var server = vertx.createNetServer()
server.listen(0, "localhost", .onComplete {
    case Success(result) => {
...
var options = NetClientOptions()
var client = vertx.createNetClient(options)
client.connect(4321, "localhost", .onComplete {
    case Success(result) => {
      println("Connected!")
      var socket = res.result()
    }
    case Failure(throwable) => {
      println(s"Failed to connect: ${res.cause().getMessage()}")
    }
  }
)

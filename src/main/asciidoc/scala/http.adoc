...
var server = vertx.createHttpServer()
server.listen(8080, "myhost.com", .onComplete {
    case Success(result) => {
...
response.push(HttpMethod.GET, "/main.js", .onComplete {
    case Success(result) => {
    ...
    case Failure(throwable) => {
      println(s"Could not push client resource ${ar.cause()}")
    }
  }
)
...
[source,scala]
----
connection.updateSettings(Http2Settings(), .onComplete {
    case Success(result) => {
...
connection.ping(data, .onComplete {
